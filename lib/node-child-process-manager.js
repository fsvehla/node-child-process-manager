...
var spawnMultiple = function (processDicts, callback) {
  ...
  processDicts.forEach(function (processDict) {
    ...
    /* This function is called whenever we have verified that the proces is up.
    *  We need it in two places, because the childprocess can be up right away,
    *  or after we spawned it */
    ...
    managedProcess.verify(100, function(error) {
      if (error) {
        ...
        logVerbose("Spawning " + managedProcess.tag);

        managedProcess.start();
        managedProcess.verify(5000, function (error) {
          logVerbose("OK: " + managedProcess.tag);
